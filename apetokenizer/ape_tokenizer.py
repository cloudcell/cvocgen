...
class APETokenizer:
    ...
    def train(
        ...
        while True:
            ...
            if merge_count >= max_merges:
                print("\rMax merges limit reached", text_padding)
                break
    ...
    def get_atomic_vocab(self, corpus):
        """
        Generates the initial atomic vocabulary from a corpus without performing BPE.

        Args:
            corpus (list): A list of strings (e.g., SELFIES or SMILES).

        Returns:
            list: A sorted list of unique atomic tokens.
        """
        print("Generating atomic vocabulary...")
        atomic_tokens = set()
        for i, sentence in enumerate(corpus):
            if i % 1000 == 0:
                print(f"Processing sentence {i}/{len(corpus)}", end="\r")
            tokens = self.pre_tokenize(sentence)
            atomic_tokens.update(tokens)
        
        print(f"\nFound {len(atomic_tokens)} unique atomic tokens.")
        return sorted(list(atomic_tokens))
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="APE Tokenizer for molecular data")
    parser.add_argument("--merge", type=int, default=0, help="Limit the number of merges (0 means none)")
    parser.add_argument("--vocab-size", type=int, default=5000, help="Maximum vocabulary size")
    parser.add_argument("--min-freq", type=int, default=2000, help="Minimum frequency for merge operations")
    parser.add_argument("--save-checkpoint", action="store_true", help="Save checkpoints during training")
    parser.add_argument("--checkpoint-path", type=str, default="checkpoint", help="Path to save checkpoints")
    parser.add_argument("--checkpoint-interval", type=int, default=500, help="Interval for saving checkpoints")
    parser.add_argument("--input-file", type=str, help="Input file containing corpus data")
    parser.add_argument("--output-dir", type=str, default="tokenizer_output", help="Directory to save the trained tokenizer")
    parser.add_argument("--type", type=str, default="selfies", choices=["selfies", "smiles"], help="Type of molecular representation")
    ...
    # Load corpus data if input file is provided
    if args.input_file:
        ...
        tokenizer.train(
            ...
            min_freq_for_merge=args.min_freq,
            save_checkpoint=args.save_checkpoint,
            checkpoint_path=args.checkpoint_path,
            checkpoint_interval=args.checkpoint_interval,
            max_merges=args.merge
        )
        ...
        # save vocabulary
        tokenizer.save_vocabulary(os.path.join(args.output_dir, "vocab.json"))
        print(f"Vocabulary saved to {os.path.join(args.output_dir, 'vocab.json')}")
    else:
        parser.print_help()
